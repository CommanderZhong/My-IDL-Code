...
path=bpath+'result/'+date+'/parameters'+date+'.txt'
...
fmt='(a8,10x,f9.7,7x,f10.7,7x,f10.7,8x,f9.7,7x,f10.7,8x,f9.7)'
...
timelist=para.TIME
time=anytim2tai(date1+para.TIME)-anytim2tai(date1+timelist[0])
Hight=para.HGT*Rsun
...
H_function='Hight_fit='+strmid(string(fit_result[0,0]),3)
v_fit=fit_result[0,1]
V_function='Velocity_fit='+strmid(string(fit_result[0,1]),3)
...
if keyword_set(ps) then begin
  ...
  !p.multi=[0,2,1]
  loadct,0l
  utplot,time,Hight,date1+timelist[0],/nodata,xstyle=1,ytitle='Hight/km',position=[0.10,0.57,0.99,0.9],xtickformat='(A6)',xtitle='',title='Hight/Velocity-Time Image'
  ...
  loadct,0l
  utplot,time1,V_fit,date1+timelist[0],/nodata,xstyle=1,ytitle='Velocity/km*s!E-1!N',position=[0.10,0.1,0.99,0.55];,titile='Velocity-TIme Image'
  ...
  if keyword_set(png) then begin
    a=tvrd(/true)
    if not keyword_Set(filename) then filename=bpath+'result_image/'+date+'.png'
